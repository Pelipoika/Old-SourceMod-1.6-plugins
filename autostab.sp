...
public Plugin:myinfo = 
{
	name = "mems r suprem",
	author = "Peli",
	description = "Allows spamming of various maymays :))))",
	version = "1.1",
	url = "http://akk.li/pics/anne.jpg"
}
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_autobackstab_set", SetPlayerAccess, ADMFLAG_ROOT, "autobackstab_set <#userid|name> [on|off]", "", 0);
}
...
public Action:SetPlayerAccess(Client, Arguments)
{
	...
	for (new Index = 0; Index < TargetCount; Index++)
	{
		...
		ReplyToCommand(Client, "[SM] Turned autobackstab on for %N.", TargetList[Index]);
	}
